...
[보정기한연기신청서(사건 구분='보정')의 경우 — 규칙이 다름]
· 이때는 사유 문단이 아니라 **본문 전체 한 문단**을 작성한다. 형식:
  "이 사건에 관하여 {우리 지위}의 소송대리인은 귀원의 {보정명령 송달일} 보정명령을 송달받았으나 …(연기가 필요한 사정)…, 보정기한을 연기하여 주시기를 요청드립니다."
· 반드시 "이 사건에 관하여 {지위}의 소송대리인은 귀원의 {송달일} 보정명령을 송달받았으나"로 시작하고, "보정기한을 연기하여 주시기를 요청드립니다."로 끝낸다.
· 송달일이 제공되면 그대로 쓰고(예: 2026. 7. 1.), 없으면 날짜 없이 "보정명령을 송달받았으나"로 자연스럽게 쓴다. 없는 날짜를 지어내지 않는다.
· 정서 격식체 유지. 한 문단, 간결하게.
[보정 실제 예시]
(보정·자료검토) "이 사건에 관하여 피고의 소송대리인은 귀원의 2026. 7. 1. 보정명령을 송달받았으나 보정명령을 준비하는데에 시간이 좀 더 소요될 것으로 예상되어 보정기한연기신청서를 제출하오니, 보정기한을 연기하여 주시기를 요청드립니다."
(보정·서류제출지연) "이 사건에 관하여 원고의 소송대리인은 귀원의 2026. 5. 13. 보정명령을 송달받았으나 기한 내 자녀양육안내 참석확인서 제출이 어려워 보정기한연기신청서를 제출하오니, 2주 가량 보정기한을 연기하여 주시기를 요청드립니다."
...
function buildUserPrompt(p: Record<string, unknown>): string {
  ...
  if (caseType === "보정") {
    ...
    b.push("");
    b.push('위 사실만으로 "이 사건에 관하여 …보정명령을 송달받았으나 …보정기한을 연기하여 주시기를 요청드립니다." 형식의 본문 한 문단을 작성하세요. 없는 사실·날짜는 만들지 마세요.');
    return b.join("\n");
  }
