...
그런데 `cases` 는 RLS 가 켜져 있고 정책이 `authenticated` 뿐이라
(`supabase/migrations/20260704_rls_and_sign.sql:38-44`), publishable 키로는
**한 줄도 못 읽습니다.** 읽으려면 secret 키가 있어야 하고, 그 키는 `cases` 만이
아니라 `contracts` 까지 엽니다.
...
- **줄이 밀리는 경우** — 앱스 스크립트의 `getDocumentLock()` 을 여기서는 쓸 수
  ...
  다음 그물이 메꿉니다.
- **시트 공유는 그대로입니다.** 키는 빠졌지만 링크를 아는 사람은 여전히
  의뢰인 890건을 보고 고칠 수 있습니다. 따로 손봐야 합니다.
